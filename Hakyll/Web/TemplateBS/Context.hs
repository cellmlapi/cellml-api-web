--------------------------------------------------------------------------------
module Hakyll.Web.TemplateBS.Context
    ( Context (..)
    , mapContext
    , field
    , constField
...
newtype Context a = Context
    { unContext :: String -> Item a -> Compiler LBS.ByteString
    }
...
--------------------------------------------------------------------------------
mapContext :: (LBS.ByteString -> LBS.ByteString) -> Context a -> Context a
...
--------------------------------------------------------------------------------
constField :: String -> LBS.ByteString -> Context a
constField key = field key . const . return


--------------------------------------------------------------------------------
functionField :: String -> ([String] -> Item a -> Compiler LBS.ByteString) -> Context a
...
--------------------------------------------------------------------------------
defaultContext :: Context LBS.ByteString
defaultContext =
...
--------------------------------------------------------------------------------
bodyField :: String -> Context LBS.ByteString
...
urlField key = field key $
    fmap (maybe LBS.empty (LBSC.pack . toUrl)) . getRoute . itemIdentifier


--------------------------------------------------------------------------------
-- | Filepath of the underlying file of the item
pathField :: String -> Context a
pathField key = field key $ return . LBSC.pack . toFilePath . itemIdentifier


--------------------------------------------------------------------------------
-- | This title field takes the basename of the underlying file by default
titleField :: String -> Context a
titleField key = field key $ return . LBSC.pack . takeBaseName . toFilePath . itemIdentifier
...
dateFieldWith locale key format = field key $ \i -> do
    ...
    return $ LBSC.pack $ formatTime locale format time
...
--------------------------------------------------------------------------------
{- Can't do this using publicly exported modules.
